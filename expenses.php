...
<main class="main-content">
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">Total: <strong style="color:var(--accent-red)"><?= formatINR($sum) ?></strong></p>
        </div>
        <button class="btn btn-primary" onclick="openModal('addTxModal')">＋ Add Expense</button>
    </div>
    <?php flash(); ?>
    ...
    <div class="filter-row">
        <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;">
            ...
            <a href="expenses.php" class="btn btn-secondary btn-sm">Reset</a>
        </form>
    </div>
    ...
    <div class="card">
        <div class="card-body">
        <?php if (empty($rows)): ?>
            <div class="empty-state"><div class="empty-icon">💸</div><p>No expenses found.</p></div>
        <?php else: ?>
            <table>
                ...
                <tr>
                    ...
                    <td><span style="background:rgba(248,81,73,0.1);color:var(--accent-red);padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600;"><?= htmlspecialchars($tx['category']) ?></span></td>
                    <td><?= htmlspecialchars($tx['description'] ?? '—') ?></td>
                    <td style="font-weight:700;color:var(--accent-red);">-<?= formatINR($tx['amount']) ?></td>
                    <td>
...
<div class="modal-overlay" id="addTxModal">
    <div class="modal">
        <div class="modal-header"><h2 class="modal-title">Add Expense</h2><button class="modal-close" onclick="closeModal('addTxModal')">✕</button></div>
...
<div class="modal-overlay" id="editTxModal">
    <div class="modal">
        <div class="modal-header"><h2 class="modal-title">Edit Expense</h2><button class="modal-close" onclick="closeModal('editTxModal')">✕</button></div>
        <form method="POST" action="actions/add_transaction.php" id="editTxForm">
            <input type="hidden" name="action" value="edit"><input type="hidden" name="type" value="expense"><input type="hidden" name="id" id="edit_id">
            <div class="form-group"><label>Category</label>
...
function deleteTx(id) {
    confirmAction('This expense will be permanently deleted.', () => {
